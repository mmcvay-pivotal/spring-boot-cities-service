...
main()
{
  ...
  cd $APPNAME
  more manifest.yml
  cat manifest.yml | sed "s/random-route: true/host: $APPNAME-$username/g" | sed "s/build\/libs\/cities-service-1.0.jar/..\/output\/cities-service-$VERSION.jar/g" > manifest.tmp
  echo ""
